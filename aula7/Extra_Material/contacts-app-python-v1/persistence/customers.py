...
def search_by_name(name: str) -> list[CustomerDescriptor]:
    with create_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT CustomerID, ContactName, CompanyName FROM Customers WHERE CompanyName LIKE ?;", '%' + name + '%')
